...
Modified for Agentarium 2026-09-06: optional complete-pose grouping before
framewise mirroring avoids equal-slot cuts. See ../SOURCE-NOTICE.md.
"""
...
from extract_strip_frames import (component_frame_groups, component_group_image,
                                 component_bounds, load_chroma_key, remove_chroma_background)
...
def main() -> None:
    ...
    with Image.open(source) as image:
        if args.method == "components":
            key = load_chroma_key(source.parent, None)
            rgba = remove_chroma_background(image, key, 96)
            groups = component_frame_groups(rgba, RUNNING_FRAME_COUNT)
            if groups is None:
                raise SystemExit("cannot recover eight complete approved gait poses")
            crops = [component_group_image(rgba, group, padding=12) for group in groups]
            slot_width = max(crop.width for crop in crops) + 24
            mirrored = Image.new("RGBA", (slot_width * RUNNING_FRAME_COUNT, rgba.height), (0, 0, 0, 0))
            for index, (group, crop) in enumerate(zip(groups, crops)):
                bounds = component_bounds(group)
                mirrored.alpha_composite(ImageOps.mirror(crop),
                    (index * slot_width + (slot_width - crop.width) // 2, max(0, bounds[1] - 12)))
        else:
